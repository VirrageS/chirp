...
var _ = Describe("Utils", func() {
	...
	It("should return proper values for Postgres config provider", func() {
		var postgres PostgresConfigProvider = config.getPostgresConfig()
		Expect(postgres.GetUsername()).To(Equal("postgres"))
		Expect(postgres.GetPassword()).To(Equal("postgres"))
		Expect(postgres.GetHost()).To(Equal("localhost"))
		Expect(postgres.GetPort()).To(Equal("5432"))
	})
