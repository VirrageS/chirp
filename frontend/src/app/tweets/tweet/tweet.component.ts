...
export class TweetComponent implements OnInit, OnDestroy {
  ...
  private _toggleFollow() {
    ...
    // send real request
    let toggleFunc = this.userService.follow(this.tweet.author.id);
    if (!this.tweet.author.following) {
      toggleFunc = this.userService.unfollow(this.tweet.author.id);
    }
  ...
  private _toggleLike() {
    ...
    let toggleFunc = this.tweetService.like(this.tweet.id);
    if (!this.tweet.liked) {
      this.tweet.like_count -= 1;
      toggleFunc = this.tweetService.unlike(this.tweet.id);
    } else {
      this.tweet.like_count += 1;
    }
